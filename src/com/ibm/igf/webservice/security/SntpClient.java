...
public class SntpClient {
    ...
    static Logger log = LogManager.getLogger(SntpClient.class);

    public static void adjustDate(Date aDate)
    {
        if (!init)
        {
            getSNTPOffset("time.nist.gov");
        }
        aDate.setTime((long) (aDate.getTime() + (localClockOffset * 1000)));
    }

    public static void getSNTPOffset(String serverName)
    {
        try
        {
            // Send request
        ...
        {
            log.fatal("Failed to set server time.  " + exc.toString());
        }
    }
}
